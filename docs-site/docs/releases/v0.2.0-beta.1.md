...
- `.github/workflows/release-replay.yml` — `workflow_dispatch` workflow
  that replays the live release-gate against any published tag,
  read-only (no `git push`, no `npm publish`, no `git tag`).
