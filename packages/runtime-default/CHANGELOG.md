...
### Changed
- No runtime or wire change vs `1.0.0`. `schemaVersion` stays `1.2.0`.
  Patch release tagged in step with the v1.0.1 hotfix line — version
  alignment only.

## [1.0.0] — 2026-05-29

### Released
- First stable release. Bit-identical to `0.2.0-rc.1`. `Invoker`,
  `PolicyEngine`, `WorkflowRuntime`, `EffectDispatcher`,
  `ProjectionRenderer`, and the ViewSchema wire shape
  (`schemaVersion 1.2.0`) are frozen as stable public surface.
